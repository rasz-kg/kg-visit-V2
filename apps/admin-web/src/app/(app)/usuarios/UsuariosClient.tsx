            ...
            <tbody>
              {list.map((u) => (
                <tr key={u.id} className="border-b border-slate-50 last:border-0 hover:bg-slate-50/60">
                  ...
                  <td className="px-5 py-3 text-slate-500">{u.username ? `@${u.username}` : "—"}</td>
              ...
              {list.length === 0 && (
                <tr><td colSpan={5} className="px-5 py-10 text-center text-slate-400">Sin usuarios en esta categoría.</td></tr>
              )}
            </tbody>
          </table>
        </div>
